/**
 * Customer Portal - Reorder Routes
 * 
...
export const reorderRoutes = new Elysia()

    /**
     * Reorder from a previous order
     */
    .post('/reorder/:orderId', async ({ headers, params, set }) => {
        ...
        if (Number(pendingCount) >= MAX_PENDING_ORDERS) {
            set.status = 400;
            return createErrorResponse('ORDER_LIMIT_REACHED');
        }
        ...
        if (originalItems.length === 0) {
            set.status = 400;
            return createErrorResponse('NO_ITEMS');
        }
        ...
        if ('error' in result && result.error) {
            ...
            return createErrorResponse(result.error.code);
        }
