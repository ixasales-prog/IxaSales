...
export const paymentGatewayRoutes = new Elysia({ prefix: '/payment-gateway' })
    ...
    .post('/webhook/click', async ({ body, set }) => {
        const params = body as ClickPrepareRequest | ClickCompleteRequest;
    ...
    .post('/webhook/payme', async ({ body, headers, set }) => {
        ...
        if (!token) {
            return paymeOrderNotFound(request.id);
        }
        ...
        if (!tenant?.paymeSecretKey) {
            return paymeInsufficientPrivilege(request.id);
        }
        ...
        switch (request.method) {
            case 'CheckPerformTransaction': {
                ...
                if (tokenInfo.token.status === 'paid') {
                    return paymeOrderAlreadyPaid(request.id);
                }
                ...
                if (request.params.amount && Math.abs(request.params.amount - expectedTiyin) > 1) {
                    return paymeAmountIncorrect(request.id);
                }

                return createPaymeResponse(request.id, {
                    allow: true,
                });
            }
