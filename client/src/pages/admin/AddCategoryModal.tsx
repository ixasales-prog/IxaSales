...
import { X, Save, Loader2, Tag } from 'lucide-solid';
...
const AddCategoryModal: Component<AddCategoryModalProps> = (props) => {
    ...
    const handleSubmit = async (e: Event) => {
        ...
        try {
            if (props.category) {
                await api.put(`/products/categories/${props.category.id}`, { name: name() });
                toast.success('Category updated successfully');
            } else {
                await api.post('/products/categories', { name: name() });
            ...
            <div class="fixed inset-0 bg-slate-950/95 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
                <div class="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-xl font-bold text-white">{props.category ? 'Edit Category' : 'New Category'}</h2>
                        <button
                    ...
                    <form onSubmit={handleSubmit} class="space-y-4">
                        <div class="space-y-1.5">
                            ...
                            <div class="relative group">
                                <Tag class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500 group-focus-within:text-blue-400 transition-colors" />
                                <input
                                    type="text"
                                    required
                                    value={name()}
                        ...
                        <div class="pt-4 flex gap-3">
                            ...
                            >
                                <Show when={!loading()} fallback={<Loader2 class="w-5 h-5 animate-spin" />}>
                                    <Save class="w-5 h-5" />
                                    {props.category ? 'Update Category' : 'Save Category'}
                                </Show>
                            </button>
                        </div>
                    </form>
